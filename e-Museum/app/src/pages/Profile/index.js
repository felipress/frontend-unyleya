...
const Profile = () => {
    ...
    const loadProfile = async () => {
        ...
        // Password Fields
        data.password = ""
        // Setting data into profile state
        setProfile(data)
    }
    ...
    const formHandle = async (event) => {
        ...
        const response = await updateUserService(userInfo.userId, profile)
        const data = await response.data
        if(data){
        ...
        <div>
            <form className="block mx-auto max-w-lg" onSubmit={formHandle}>
                ...
                <div className="my-3 flex flex-col">
                    ...
                    <input type="password" name="password" required="required" onChange={onChangeHandle} value={profile.password} className="block px-3.5 py-2.5 rounded border border-solid w-full  focus:outline-orange-500" />
                </div>
